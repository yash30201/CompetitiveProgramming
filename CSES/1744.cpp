...
// return dp[r][c] = 0;
// int res = INF;
// FOR(i, 1, r) res = min(res, recurr(i, c) + recurr(r - i, c));
// FOR(j, 1, c) res = min(res, recurr(r, j) + recurr(r, c - j));
// return dp[r][c] = res + 1;
// }
...
void solve() {
    ...
    FOR(i, 1, n) FOR(j, 1, m) {
        if (i == 1)
            ...
            int x = INF;
            FOR(ii, 1, i) x = min(x, dp[ii][j] + dp[i - ii][j]);
            FOR(jj, 1, j) x = min(x, dp[i][jj] + dp[i][j - jj]);
            dp[i][j] = x + 1;
        }
    }
    op(dp[n - 1][m - 1]);
