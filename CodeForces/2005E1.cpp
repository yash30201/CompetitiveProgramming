...
int dp[7][MAXN][MAXN], n, m, L;
int solver(vector<vi> &b, vi &a, int r, int c, int l) {
    if (l == L || r == n || c == m) return 0;
    if (dp[l][r][c] != -1) return dp[l][r][c];
    if (b[r][c] == a[l]) dp[l][r][c] = 1 - solver(b, a, r + 1, c + 1, l + 1);
    if (dp[l][r][c] == 1) return 1;
    return dp[l][r][c] = (solver(b, a, r + 1, c, l)
        || solver(b, a, r, c + 1, l));
}
