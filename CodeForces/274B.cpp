...
// We'll return net of positive and negative
// numbers to represent each subtree
// {positive, negative}
pll DfsWithDp(vi g[], int node) {
    vis[node] = true;
    ll pos = -INFL, neg = INFL;
    for (int i: g[node]) {
        if (vis[i]) continue;
        auto [p, n] = DfsWithDp(g, i);
        pos = max(pos, p);
        neg = min(neg, n);
    }
    if (pos == -INFL) pos = 0LL;
