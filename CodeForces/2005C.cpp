...
void solve() {
    ...
    string narek = "narek";
    vi dp(5, -INF);
    // Since we are looking for letter n right now, that means we can start
    // afresh, and don't really need to rely on previous letter for sure, its
    // just a good to have the answer for previous letter end at 'k'
    dp[0] = 0;
    FOR(i, 0, n) {
        int tot = 0;
        vi tmp = dp;
        FOR(j, 0, m) {
            if (narek.find(s[i][j]) != string::npos) {
                tot++;
            }
        }
        // see(i, tot);
        FOR(l, 0, 5) {
            if (tmp[l] == -INF) continue;
            int ni = l, score = 0;
            FOR(j, 0, m) {
                if (s[i][j] == narek[ni]) {
                    ni++;
                    ...
                    dp[ni] = max(dp[ni], tmp[l] - tot + score * 10);
                }
            }
        }
    }
