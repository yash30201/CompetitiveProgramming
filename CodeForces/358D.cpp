...
int SolveForRange(_v<vi> &a, int prev_fed, int idx) {
    if (dp[prev_fed][idx] != -1) return dp[prev_fed][idx];
    // see(prev_fed, idx);
    if (idx == (int)a[0].size() - 1) {
        return dp[prev_fed][idx] = a[prev_fed][idx];
    }
    int res;
    if (prev_fed) {
        res = a[1][idx] + SolveForRange(a, 1, idx + 1);
        res = max(res, a[2][idx] + SolveForRange(a, 0, idx + 1));
    } else {
        res = a[0][idx] + SolveForRange(a, 1, idx + 1);
        res = max(res, a[1][idx] + SolveForRange(a, 0, idx + 1));
    }
    return dp[prev_fed][idx] = res;
}
...
void solve() {
    ...
    vector<vi> a(3, vi(n));
    ip(a);
    op(SolveForRange(a, 0, 0));
    // FOR(i, 0, 2) {
    //     FOR(j, 0, n) cerr << dp[i][j] << ' ';
    //     cerr << endl;
    // }
