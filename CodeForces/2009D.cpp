...
void solve() {
    ...
    ll res = 0;
    int x = 0;
    if (!o.empty() && binary_search(all(z), o[0])) x++;
    FOR(i, 1, sz(o)) {
        res += x;
        if (binary_search(all(z), o[i])) {
            res += i;
            x++;
        }
    }
    auto get = [](vi &one, vi &zero) {
        int rr = 0;
        FOR(i, 0, sz(one)) {
            int l = one[i] - 1, r = one[i] + 1;
            if (binary_search(all(zero), l) && binary_search(all(zero), r)) rr++;
        }
        return rr;
    };
    res += get(o, z);
