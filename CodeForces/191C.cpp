...
class LCA {
    ...
    void dfs(int node) {
        FOR(b, 1, B) {
            par[b][node] = par[b - 1][par[b - 1][node]];
        }
        start_time[node] = ++id;
        for (auto [to, id]: g[node]) {
            if (par[0][to] != -1) continue;
            par[0][to] = node;
            dfs(to);
        }
        end_time[node] = ++id;
    }
    inline bool IsParent(int x,/*of=*/ int y) {
        return (start_time[x] < start_time[y])
...
public:
    ...
    int get(int x, int y) {
        ...
        FORD(b, 0, B) {
            if (!IsParent(par[b][x], y)) x = par[b][x]; 
        }
        return par[0][x];
    }
};
