...
void bfs(vi g[], vi& dist, int source) {
    dist[source] = 0;
    queue<int> q;
    q.push(source);
    while (!q.empty()) {
...
void solve() {
    ...
    vi d1(n + 1, -1), dn(n + 1, -1);
    bfs(g, d1, 1);
    bfs(g, dn, n);
    int normal = d1[n];
    vi suff(k);
    sort(all(sp), [&](int x, int y){
        return d1[x] < d1[y];
    });
    suff[k - 1] = dn[sp[k - 1]];
    FORD(i, 0, k - 1) suff[i] = max(dn[sp[i]], suff[i + 1]);
    int best = 0;
    FOR(i, 0, k - 1) {
        int x = d1[sp[i]] + suff[i + 1] + 1;
        if (x >= normal) {
            op(normal);
            return;
        }
