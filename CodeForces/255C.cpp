...
void solve() {
    ...
    int res = 0;
    auto MaxIntersection = [](const vi &x, const vi &y) {
        int i = 0, j = 0;
        bool turn_x = true;
        int p = -1;
        int res = 0;
        auto DoTurn = [&p, &res](const vi &a, int &idx) -> bool {
            while (idx < sz(a) && a[idx] <= p) idx++;
    ...
    for (auto it1 = mp.begin() ; it1 != mp.end() ; it1++) {
        auto &v1 = it1->second;
        res = max(res, (int)sz(v1));
        auto it2 = it1;
        for (++it2 ; it2 != mp.end() ; ++it2) {
            auto &v2 = it2->second;
            see(v1, v2);
            res = max(res, MaxIntersection(v1, v2));
            res = max(res, MaxIntersection(v2, v1));
        }    
    }
    op(res);
    return;
