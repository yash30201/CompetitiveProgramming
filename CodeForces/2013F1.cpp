...
int dfs2(vi g[], vb &vis, int node) {
    int res = 0;
    vis[node] = true;
    for (int to : g[node]) {
        if (vis[to]) continue;
        res = max(res, dfs2(g, vis, to) + 1);
    }
    return res;
}

void SpecialBfs(vi g[], vi& path, ump<int, int> &mp, int n)  {
    vb vis(n + 1, false);
    for(int i: path) vis[i] = true;
    for (int i: path) mp[i] = dfs2(g, vis, i);
} 
void solve() {
    // Let's begin
    int n;
    ...
    dfs(g, par, 1);
    vi path;
    for (int node = u ; node != 1 ; node = par[node]) {
        path.push_back(node);
    }
    path.push_back(1);
    reverse(all(path));
    int m = path.size();
    ump<int, int> mp;
    SpecialBfs(g, path, mp, n);
    int l = 0, r = m - 1;
    bool alice_turn = true;
    stack<int> bob_maxes;
    FORD(i, 1, m) {
        int x = m - i - 1 + mp[path[i]];
        if (bob_maxes.empty()) bob_maxes.push(x);
        else bob_maxes.push(max(x, bob_maxes.top()));
    }
    while (l < (m + 1) / 2) {
        int max_alice = l + mp[path[l]];
        if (max_alice > bob_maxes.top()) {
            op("Alice");
            return;
        }
        l++;
