...
void solve() {
    ...
    while (n--) {
        ...
        if (type == 1) {
            ...
            ip(k);
            len++;
            ft.rangeAdd(len, len, k);
        } else {
            int y = ft.get(len, len);
            ft.rangeAdd(len, len, -y);
            len--;
        }
        double z = static_cast<double>(ft.get(0, len));
        cout << fixed << setprecision(6) << z / len << endl;
    }
    return;
}
