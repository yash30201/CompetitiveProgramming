...
// N <= 100, width <= 100, thickness <= 2
// Aim is to minimize the width
...
void solve() {
    ...
    int T = 0;
    FOR(i, 0, n) {
        cin >> books[i].thickness >> books[i].width;
        T +=  books[i].thickness;
    }
    // Min Width with thickness T
    vi dp(T + 1, INF);
    dp[0] = 0;
    FOR(i, 0, n) {
        vi prev = dp;
        fill(all(dp), INF);
        FOR(j, 0, T + 1) {
            // Put the book vertically
            if (j + books[i].thickness <= T) {
                dp[j + books[i].thickness] = min(
                    dp[j + books[i].thickness],
                    prev[j]
                );
            }

            // Put the book horizontally
            dp[j] = min(dp[j], prev[j] + books[i].width);
        }
    }
    // see(dp);
    FOR(i, 0, T + 1) {
