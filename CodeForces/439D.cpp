...
void solve() {
    ...
    ip(a,b);
    int r = INF + 1, l = 0;
    auto TotalOperations = [&](int x) -> ll {
        ll res = 0;
        FOR(i, 0, n) res += max(0, x - a[i]);
        FOR(i, 0, m) res += max(0, b[i] - x);
        return res;
    };
    while (r - l > 4) {
        int d = (r - l) / 3;
        int m1 = l + d, m2 = r - d;
        ll fm1 = TotalOperations(m1 - 1), fm2 = TotalOperations(m2);
        if (fm1 >= fm2) l = m1;
        if (fm2 >= fm1) r = m2;
    }
    ll res = INFL;
    FOR(i, l, r) {
        res = min(res, TotalOperations(i));
    }
    op(res);
    return;
}
